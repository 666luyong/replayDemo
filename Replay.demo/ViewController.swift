...
class ViewController: UIViewController {
    ...
    func showContent()  {
        ...
        let fileURLString : String = url.absoluteString.appending((contentsOfPath?.first)! ).replacingOccurrences(of: "file:///", with: "")
        //let fileURLString  = Bundle.main.path(forResource: "simple-drum-beat", ofType: "wav")!
        let file = EZAudioFile(url: URL(fileURLWithPath: fileURLString))
        guard let data = file?.getWaveformData() else {
            return
        }
        let waveform = EZAudioPlot()
        self.view.addSubview(waveform)
        waveform.frame = (plot?.frame)!
        waveform.plotType = EZPlotType.buffer
        waveform.shouldFill = true
        waveform.shouldMirror = true
        waveform.color = UIColor.red
